...
COHA_SENSE_ANALYSIS_DIR = './data/COHA/hidden1-maskedN/sense_analysis_all10nn'
COHA_SENSE_ANALYSIS_SUMMARY_DIR = './data/COHA/hidden1-maskedN/sense_analysis_summary_all10nn'
COHA_SENSE_REGRESSION_DIR = './data/COHA/hidden1-maskedN/sense_regression'
COHA_CDIV_DIR = './data/COHA/hidden1-maskedN/cdiv'
...
def get_factor_vals(matches_idx):
    ...
    sense_matches = list(pd.read_csv(os.path.join(HU19_DIR, 'matches', f'hu19_sense_matches_{matches_idx}.csv')).to_records(index=False))
...
def run_logit(matches_idx, factor_vals):
    ...
    """
    factor_diffs_half = {}
    n_half = len(factor_vals['sem_dens']['dec']) // 2  # all have same num, 414/2 = 212
    for factor in factor_vals:
...
def analyze_HU19_sense_factors():
    ...
    for i in tqdm(range(10)): # range(1)
